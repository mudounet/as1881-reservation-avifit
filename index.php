...
foreach (
    $xml->xpath("//insc[ translate(@date,'-','') <  $dateDeNettoyage ]")
    as $el
) {
...
foreach (
    $wl->xpath("//wl[ translate(@date,'-','') <  $dateDeNettoyage ]")
    as $el
) {
...
if ($_GET["act"] == "remove" && $GP_name != "" && $GP_email != "") {
    ...
    foreach ($wl->xpath('//wl[@date="' . $GP_date . '"]') as $el) {
        ...
        $to = $el["email"];
        $subject =
            "AS1881 - Une place vient de se libérer pour le " .
            $dateHuman .
            " !";
        $message = $mailBody;

        $headers =
            "From: [TEST !!!] AS1881 - Avifit <avironstrasbourg1881@gmail.com >" .
            "\r\n";
